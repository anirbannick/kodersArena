...
def readConfigFile():
     ...
     for line in lines:
          ...
          elif tokens[0] == 'list of easy questions':
               listofEasy = tokens[1].split(',')
               print listofEasy
               print
          elif tokens[0]== 'list of medium questions' :
               listofMedium = tokens[1].split(',')
               print listofMedium
               print
          elif tokens[0] == 'list of hard questions' :
               listofHard = tokens[1].split(',')
               print listofHard
               print
          else:
                raise Exception
...
class Score:
     ...
     def __init__(self, row):
          if row != None:
               self.tname, self.score, self.solved, self.sub, self.timestamp,self.noqsolved, self.rank = row
     def show(self):
          print 'tname=', self.tname
...
def updateScore(tname, addPts, solved,noqsolved):
     ...
     query = "UPDATE score SET noqsolved = '%s' WHERE tname= '%s'" % (noqsolved,tname)
     cursor.execute(query)
...
def Processor(threadNum):
     ...
     while True:
          ...
          j = iq.get()
          print 'In Processor class, job := %d' % threadNum
          j.show()
          ...
          if solved.find(prob) >= 0:
               ...
               if str(j.prob_no) in listofEasy:
                    addPts = ptsEasy
               elif str(j.prob_no) in listofMedium:
                    addPts = ptsMed
               elif str(j.prob_no) in listofHard:
                    addPts = ptsHard
               else:
                    raise Exception
               ...
               try:
                   ...
                   print 'Code file saved to user_codes'
               except IOError, e:
          ...
          writeDB(query)
          updateScore(j.tname, add, solved,nsolved)
